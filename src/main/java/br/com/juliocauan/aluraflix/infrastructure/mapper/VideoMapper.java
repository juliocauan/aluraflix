...
public abstract class VideoMapper implements BaseMapStruct<VideoEntity, VideoGet, VideoPost, VideoPut> {
    ...
    @Override
    @Mapping(target = "categoria", expression = "java(categoriaService.findOneOrGetDefaultId(postDto.getCategoriaId()))")
    public abstract VideoEntity postDtoToEntity(VideoPost postDto);

    @Override
    @Mapping(target = "categoria", expression = "java(putDto.getCategoriaId() == null ? null : categoriaService.findOneOrBadRequest(putDto.getCategoriaId()))")
